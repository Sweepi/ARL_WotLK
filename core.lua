...
function private.SetTextColor_ColorCodeWithAlpha(color_code_with_alpha, text)
	return string.format("|c%s%s|r", color_code_with_alpha or "ffffffff", text)
end
			       ...
			       for spell_id in pairs(unit.item_list) do
				       ...
				       if scanned then
					       ...
					       if ((not recipe:HasState("KNOWN") and has_level) or shifted) and Player:HasRecipeFaction(recipe) then
							...
							--    print(string.format("%s: %s%s|r (%d)", recipe.profession, hex, recipe.name, recipe.skill_level))
						       count = count + 1
					       end
				       end
...
function addon:OnEnable()
	...
	elseif MRTUIUtils_RegisterWindowOnShow then
		-- print("ARL: MRTUIUtils_RegisterWindowOnShow")
...
do
	...
	function addon:TRADE_SKILL_SHOW()
		...
		if scan_parent == TradeSkillFrame then
			print("ARL: TradeSkillFrame")
			scan_button:ClearAllPoints()

			local loc = addon.db.profile.scanbuttonlocation

			if loc == "TR" then
				scan_button:SetPoint("RIGHT", TradeSkillFrameCloseButton, "LEFT",-200,0)
				-- scan_button:SetPoint("LEFT", TradeSkillFrameCloseButton, "LEFT",4,0)
			elseif loc == "TL" then
...
do
	...
	local function Recipe_GetDisplayName(recipe_entry)
		...
		-- print(quality_color)
		local recipe_string = private.SetTextColor_ColorCodeWithAlpha(quality_color, recipe_name)
...
do
	...
	copy_frame:SetBackdrop(backdropInfo)
